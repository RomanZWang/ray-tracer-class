 ...
 * is always assumed to be Scene).  If {@literal <}name{@literal >} is the name of
 * the node, the class type of the parent node is searched for methods
 * named, first, set {@literal <}name{@literal >} and then add {@literal <}name{@literal >}.  If no such method is found
 ...
 * attribute in the node.  The value of the type node can be the fully 
 * qualified name for class (e.g. ray.surface.Sphere), or just the name
 * of a class in the same package as the base type inferred from the parent 
 * method (e.g. type="Sphere" when the addSurface method accepts type 
 * ray.surface.Surface, is shorthand for type="ray.surface.Sphere").
 * Additionally, nodes can be named with the "name" attribute.  After they
 * are added to the parent, named nodes are added to a hash table internal 
 * to the parser.  Later, other nodes can reference a named node by using a 
 * "ref" attribute.  The value of the  reference attribute is used to index 
 * in the hash table nd the previously stored node with the matching name is 
 * used instead of a parsed value.
 *
 ...
 *      <eye> 1 1 1 </eye>
 *    </camera>
 *    <material name="white" type="ray.material.Lambertian">
 ...
 *  -- Set camera takes a Camera as an argument, so the {@literal <}camera{@literal >}
 *     node is parsed as a Camera object
 *  -- The {@literal <}eye{@literal >} node is processed similarly, finding setEye
 *     in camera, but setEye takes a Vector3 as an argument
 *     so its value is parsed directly as a Vector3 (whitespace
 *     delimited)
 *  -- After {@literal <}eye{@literal >} is processed, setEye(eye) is called in Camera
 *  -- Then setCamera(camera) is called on Scene
 *  -- The {@literal <}material{@literal >} tag is processed in the same fashion, but
 ...
 *     later use in the {@literal <}surface{@literal >} tag.  The {@literal <}material{@literal >}
 *     tag in the {@literal <}surface{@literal >} tag is not parsed, but the same object
 *     produced by parsing the above "white" material is supplied
 *     instead.
 *  -- The type of the {@literal <}surface{@literal >} tag is shorthand for 
 *     type="ray.surface.Sphere".
 *
...
public class Parser {
	...
	/** 
	 * Parser will assume all that non-absolute paths for mesh files are relative to
	 * the path of the xml file itself.  This string stores that path 
	 * and it is appended infront of the path's to all xml files.
	 */
	...
	private Object parseObject(Class<?> c, String text) {
		...
		if (c == String.class) {
			...
			if (tempArray.size() != 3) {
				throw new Error("Tuple3 is not of length 3 (" + tempArray.size() + ")");
			}
			Tuple3 result;
			try {
			...
			try {
				...
				result = (Image) imageConstructor.newInstance(new Object[] { (Integer) tempArray.get(0), (Integer) tempArray.get(1) });
			}
			catch (Exception e) {
				throw new Error("Error instantiating object of class: " + c.getName());
			}
	...
	private Object parseObject(Class<?> c, Node n) {
		...
		if (typeAttribute != null) {
			...
			try {
				...
				try {
					...
					possibleClass = Class.forName(c.getPackage().getName() + "." + className);
				}
		...
		// Otherwise the node represents a general object
			...
			for (int i = 0; i < children.getLength(); i++) {
				...
				if (foundMethod == null) {
					throw new Error("Could not find a method to use to add " + childName + " to the class type " + c.getName() + ".");
				}
				
				// Check that the method has the right number of parameters
				Class<?>[] parameterTypes = foundMethod.getParameterTypes();
				if (parameterTypes.length != 1) {
					throw new Error("Method " + foundMethod.getName() + " must take exactly one parameter.");
				}
				...
				try {
					...
					//Append the path string for meshes
					if(c == Mesh.class && foundMethod.getName().equalsIgnoreCase("setData")) {
						
						//Check that the give path is not absolute
						File testFile = new File((String) childValue);
						if(!testFile.isAbsolute())
							childValue = basePath + ((String) childValue);
					...
					System.err.println("Error invoking the method " + foundMethod.getName() + ".");
					e.printStackTrace();
				}
			}
		}
	...
	public Object parse(String filename, Class<?> c) {
		...
		Object result = null;
		try {
			basePath = (file.getParent() == null ? "" : file.getParent()+File.separator);
