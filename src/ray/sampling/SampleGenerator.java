...
public interface SampleGenerator {
	 ...
	 * 0 {@literal <}= row {@literal <} {@link #getNumSamples()}.  Two samples drawn using the same 
	 * index are not uniformly distributed (for example, they might always be 
	 * the same point). 
	...
	/**
	 * Clone this sample generator for use in an individual thread.
	 * Some sample generator might have to maintain its internal data structure, which might
	 * not be shared across multiple threads
	 * 
	 * @return a new copy of sample generator
	 */
	SampleGenerator thread_safe_clone();
}
