...
public class Geometry {
	...
	/**
	 * @param seed A point randomly distributed in the unit square. seed.x and see.y are two
	 * independently and uniformly sampled random numbers in [0,1].
	...
	static public void squareToPSAHemisphere(Point2 seed, Vector3 outDir) {
		...
		double theta = Math.asin(r);
		double phi = v * 2 * Math.PI;

		outDir.setSpherical(theta, phi);

		outDir.x = r * Math.cos(phi);
		outDir.y = r * Math.sin(phi);
		outDir.z = Math.sqrt(1-u);
	}
	...
	static public void squareToHemisphere(Point2 seed, Vector3 outDir) {
		...
		outDir.x = Math.sqrt(1-Math.pow(u,2)) * Math.cos(phi);
		outDir.y = Math.sqrt(1-Math.pow(u,2)) * Math.sin(phi);
		outDir.z = u;
	}
